...
public class Solution2 {
	...
	public static void main(String[] args) {
		...
		//Check if each machine is winnable and minimum tokens needed to win
		...
		// for(int i = 0; i < clawMachines.size(); i++) {
			...
			// long tokens = findMinTokens(mach);
	...
	public static void readFile(Scanner fin, ArrayList<Coords[]> claw) {
		...
		while(fin.hasNextLine() ) {
			...
			machineCoords[0] = new Coords( Double.parseDouble( btnA[0].substring(3) ),
																		 Double.parseDouble( btnA[1].substring(3) ) );
			machineCoords[1] = new Coords( Double.parseDouble( btnB[0].substring(3) ),
																		 Double.parseDouble( btnB[1].substring(3) ) );
			machineCoords[2] = new Coords( Double.parseDouble( prize[0].substring(3) ),
																		 Double.parseDouble( prize[1].substring(3) ) );
	...
	public static long findMinTokens(Coords[] mach) {
		final long ACOST = 3, BCOST = 1;
		final double TOLERANCE = 0.001d;
		double ax = mach[0].x, ay = mach[0].y;
		double bx = mach[1].x, by = mach[1].y;
		double px = mach[2].x + 10000000000000d, py = mach[2].y + 10000000000000d;
		
		//Solve System of equations
		//Solve intermediate value for b presses in terms of a presses
		//Use resulting equation to solve for a presses
		
		double rightSide = py - by * px / bx;
		double leftSide = ay - by * ax / bx;
		
		double a = rightSide / leftSide;
		
		//Use value of a presses to solve for b presses
		double b = (px - ax * a) / bx;
		
		//Compute tolerance for double values
		double aTol = Math.abs(a - Math.round(a) );
		double bTol = Math.abs(b - Math.round(b) );

...
public class Solution2Sim {
  
  static final int FPS = 1000;
  static final int SLEEPDELAY = 1000/FPS;
  static final String ROBOTCOLOR = "\033[1;33m";
  static final String BOXCOLOR = "\033[1;32m";
  static final String WALLCOLOR = "\033[0;31m";
  static final String BACKGROUNDCOLOR = "\033[0;40m";
  static final String EMPTYCOLOR = "\033[0;30m";
  static final String TEXTCOLOR = "\033[1;97m";
  
  static String prevFrame = null;
    ...
    System.out.print(BACKGROUNDCOLOR);
		System.out.println(TEXTCOLOR + "INITIAL STATE:");
		prevFrame = printGrid(grid, prevFrame);
    
    //Move the robot according to all instructions in the moveset
		for(int i = 0; i < moveset.length(); i++) {
			moveRobot(grid, robotLoc, moveset.charAt(i) );
			System.out.println(TEXTCOLOR + "AFTER MOVING: " + moveset.charAt(i) );
			prevFrame = printGrid(grid, prevFrame);
      sleep(SLEEPDELAY);
		}//end for
		...
		System.out.println("\033[" + (grid.size() + 2) + ";0H" + TEXTCOLOR + totalBoxCoords);
	}//end main
  
  public static void clearScreen() {
    //Return cursor to top-left, clear screen, & hide cursor
    System.out.print("\033[H\033[2J\033[?25l");
    System.out.flush();
  }//end clearScreen
  
  public static String printGrid(ArrayList<StringBuilder> grid, String prev) {
    StringBuilder next = new StringBuilder();
    String[] previous = prev != null ? prev.split("\n") : null;
    ...
    for(int i = 0; i < grid.size(); i++) {
      ...
      for(int j = 0; j < grid.get(0).length(); j++) {
        ...
        switch(ch) {
          case '[':
          case ']':
            row.append(BOXCOLOR + ch);
            break;
          case '#':
            row.append(WALLCOLOR + ch);
            break;
          case '@':
            row.append(ROBOTCOLOR + ch);
            break;
          case '.':
            row.append(EMPTYCOLOR + ch);
        }//end switch
      }//end for
      
      row.append('\n');
      next.append(row);
      if(prev == null || previous[i].equals(row) ) continue;
      //Only update rows that have changed. 
      //\033[r;cH places terminal cursor at [r][c], with 1-based indexing
      System.out.println("\033[" + (i + 2) + ";0H" + row);
    }//end for
  ...
  public static void sleep(int ms) {
    ...
    System.out.print("\033[H"); //Return cursor to top-left
  }//end sleep
  
  public static void restore() {
    //Restore terminal text settings & unhide cursor

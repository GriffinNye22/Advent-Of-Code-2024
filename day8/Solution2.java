...
public class Solution2 {
  ...
  public static int calcAntinodes(Map<Character, ArrayList<Location> > ant, int[] dim) {
    Set<Location> antinodes = new LinkedHashSet<>(); //Set of all antinode locations
    int numAntSites = 0;
    ...
    for(Map.Entry<Character, ArrayList<Location> > entry : ant.entrySet() ) {
      ...
      for(int i = 0; i < coords.size() - 1; i++) {
        ...
        for(int j = i + 1; j < coords.size(); j++) {
          ...
          while( isValidCoord(r1, c1, dim) ) {
            antinodes.add( new Location(r1, c1) );
            r1 += r;
            c1 += c;
          }//end if
          
          while( isValidCoord(r2, c2, dim) ) {
        ...
        antinodes.add( coords.get(i) ); //Add antenna site to antinodes
      }//end for
      
      antinodes.add( coords.get( coords.size() - 1) ); //Add final antenna site to antinodes
    }//end for
    
    
    for(Location entry : antinodes) {
      System.out.println(entry.r + "," + entry.c);
    }//end for      

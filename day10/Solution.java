...
public class Solution {
	...
	public static void main(String[] args) {
		...
		//Traverse all possible trails and tally total scores of all trails
		//Where scores represent the total number of trail ends that can be reached
		//from a trailhead.
		for(Coords trail : trailheads) {
			totalScore += traversePath(grid, trail, new HashSet<>() );
		}//end for
	...
	public static int traversePath(ArrayList< ArrayList<Integer> > grid, Coords start, Set<Coords> visited) {
		...
		for(DIR d : DIR.values() ) {
			...
			if( inBounds(nextR, nextC, height, width) && grid.get(nextR).get(nextC) == curr + 1) {
				traversePath(grid, new Coords(nextR, nextC), visited);
			}//end if
		}//end for

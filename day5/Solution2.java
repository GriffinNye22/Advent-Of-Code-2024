...
public class Solution2 {
    ...
    //Sum middle page numbers of all incorrect updates after fixing them
    result = calcIncorrectMiddlePageNumbers(pageMap, orderList);
  ...
  public static int calcIncorrectMiddlePageNumbers(Map<Integer, Set<Integer> > pages, ArrayList<Integer[]> updates) {
    int pageNumbers = 0;
    ...
    for(int i = 0; i < updates.size(); i++) {
      ...
      //Fix invalid updates and sum middle page numbers
      if (!isValidUpdate(masterOrder, currUpdate) ) {
        Integer[] properOrder = new Integer[masterOrder.size()];
        
        for(Map.Entry<Integer, Integer> entry: masterOrder.entrySet() ) {
          int node = entry.getKey();
          int idx = entry.getValue();
          properOrder[idx] = node;
        }//end for
        
        pageNumbers += properOrder[properOrder.length / 2];
      }//end if
    }//end for

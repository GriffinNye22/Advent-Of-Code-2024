...
class Grid {
	...
	public void markVisited(Node n) {
		if(n == null) return;
		if(strGrid[ n.getR() ].charAt(n.getC()) == CORRUPTED) System.out.println("YOU DONE FUCKED UP");
...
public class Solution {
	...
	public static void main(String[] args) {
		...
		//Corrupt fallen bytes in the memory space
		corruptedNodes.stream().limit(NUMFALLEN).forEach( i -> memorySpace.corruptNode(i[0], i[1]) );
		...
		while(!pq.isEmpty() ) {
			...
			for(Node neighbor : neighbors) {
				...
				if( nbrEntry.getValue() < processed.getOrDefault(neighbor, -1) ) {
					if(nbrEntry.getKey().getCost() != neighbor.getCost() )System.out.println("MISMATCH!");
					processed.put(neighbor, nbrEntry.getValue() );
					continue;
				}//end if
				
				//If neighbor already in pq or processed, but already cheaper, move to next neighbor
				if( pqMap.containsKey(neighbor) || processed.containsKey(neighbor) ) /*{ System.out.println("BETTER COST EXISTS: SKIPPING!");*/ continue; //}
...
//in the Dijkstra's problem for day 16 part since it's currently kicking my ass for no reason.
